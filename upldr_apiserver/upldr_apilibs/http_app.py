...
from .cluster_manager.cluster import Cluster
import platform
import json
import socket
import threading
...
class HttpApp:
    @staticmethod
    def start_app(host="localhost", port=25565, start_cluster=False):
        logging = Util.configure_logging(name=__name__)
        server_address = (host, port)
        if start_cluster:
            logging.info("Setting up cluster...")
            cluster = Cluster()
            threading.Thread(target=cluster.start).start()
            ClusteredServerObject.cluster = cluster
            httpd = ThreadingHTTPServer(server_address, ClusteredServerObject)
        else:
